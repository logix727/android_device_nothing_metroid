...
## Installed baseline (r4, `23.0-20260806-UNOFFICIAL-metroid`)
...
- eUICC is not advertised on r4 (no LPA/EuiccService installed).
- Slot A, Enforcing, encrypted, slot marked successful, empty crash buffer.
...
2. Boot-image security-patch level still blank (MTR-007); platform `2026-02-01`
   and vendor `2025-06-05` are populated.
3. Redistribution clearance for the proprietary IMS package before any public
...
4. Release-reproducibility and record-consistency audits for every future release
   (r4 record remediation is complete for this snapshot).
