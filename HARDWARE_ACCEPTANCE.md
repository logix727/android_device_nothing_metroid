...
| VoLTE/VoWiFi/emergency/DSDS | BLOCKED | FAIL/PARTIAL: successor QTI services run, but external capture leaves framework IMS unregistered with voice/SMS capabilities false despite a contradictory vendor `VOLTE ims registered` message | Validate MMTEL registration, calls, IMS SMS, two SIMs and handover on the frozen r30 ROM/modem pair. |
