...
| Removable eUICC active profile | BLOCKED | PARTIAL/FAIL on historical early build: eUICC/EID, subscription, LTE and bidirectional SMS pass; profile refresh/removability metadata and data fail | Install the slot-removability correction, then repeat active-profile, management and coexistence tests. |
...
| VoLTE/VoWiFi/emergency/DSDS | BLOCKED | r35 contains AT&T carrier policy and APNs, but its framework device IMS capability gates remain false; Swedish r35 also lacks carrier-ID 1691 IMS/XCAP rows | Install the stock-backed framework/APN corrections, then confirm effective gates, IMS PDN/registration, calls, IMS SMS, two SIMs and handover on matching modem firmware. |
| eSIM UI/QR path | PASS through QR scanner | PARTIAL: external removable eUICC proves active-profile use; built-in eUICC hardware is not established; profile refresh fails | Real removable-eUICC credentials: download, enable, reboot, delete and transfer. |
