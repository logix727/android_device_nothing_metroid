...
| eSIM UI/QR path | PASS through QR scanner | r44 diagnostic PASS: a real Dark Star profile downloaded and enabled through the native LPA; incompatible public TS.48 code correctly reached SM-DP+ and returned CI mismatch `8.8.2/3.1` | Repeat real download on a coherent candidate only if a transferable credential is available; otherwise preserve the installed profile and test management/reboot. |
