...
| Cellular voice/SMS/MMS/data | FIX COMMITTED, NOT INSTALLED | External physical-SIM capture reaches LTE home service and outbound non-IMS SMS reaches the recipient, but replies are not received; CarrierResolver identifies specific carrier `10028` while r30 lacks its stock `nrphone` profiles, so generic APNs receive modem `0x1004`; outgoing CS call independently receives cause 252 `Explicit network reject` | Install a coherent candidate containing `vendor/apn` `ed04060`; confirm `nrphone` selection, then repeat data, inbound/outbound SMS, MMS and calls on matching modem firmware. |
| VoLTE/VoWiFi/emergency/DSDS | FIX COMMITTED, NOT INSTALLED | External capture leaves framework IMS unregistered and voice/SMS capabilities false; framework deterministically disables voice MMTEL because Lineage omits stock AT&T VoLTE availability policy | Install the metroid CarrierConfig RRO with APN commit `ed04060`; confirm effective VoLTE availability, IMS PDN/registration, calls, IMS SMS, two SIMs and handover on matching modem firmware. |
